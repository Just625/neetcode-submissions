class Solution {
    public int[] topKFrequent(int[] nums, int k) {
        ...
        // 2. Min-heap
        // Time O(n + nlogk + k) -> O(nlogk), Space O(n + k)
        // Only care about k-th elements -> no need to sort everything
        int[] ans = new int[k];
        Map<Integer, Integer> freq = new HashMap<>();
        for (int num : nums) {
            freq.put(num, freq.getOrDefault(num, 0) + 1);
        ...
        // Compare by frequency which is value in Map -> O(logk)
        // Keep the k elements with the highest frequencies. 
        // The smallest frequency among them stays at the top so it can be poll out easily
        PriorityQueue<Map.Entry<Integer, Integer>> heap = new PriorityQueue<>((f1, f2) -> f1.getValue().compareTo(f2.getValue()));
        for (Map.Entry<Integer, Integer> entry : freq.entrySet()) {
            heap.offer(entry);
            // The polled element is the smallest element
            if (heap.size() > k) {
                heap.poll();
            }
        }
