class Solution {
    public int[] productExceptSelf(int[] nums) {
        ...
        // Remember for every number: its product = all numbers before it x all numbers after it = prefix * suffix
        // One loop going from start to end to compute prefix 
        // Other loop going from end to start to compute suffix
        int len = nums.length;
        int[] ans = new int[len];
        int currentPrefixProducts = 1;
        int[] prefix = new int[len];
        for (int i = 0; i < len; i++) {
            prefix[i] = currentPrefixProducts;
            currentPrefixProducts *= nums[i];
        }

        int currentSuffixProducts = 1;
        int[] suffix = new int[len];
        for (int i = len - 1; i >= 0; i--) {
            suffix[i] = currentSuffixProducts;
            currentSuffixProducts *= nums[i];
        }
