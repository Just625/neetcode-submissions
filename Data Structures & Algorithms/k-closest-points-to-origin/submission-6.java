class Solution {
    public int[][] kClosest(int[][] points, int k) {
        ...
        // Time O(nlogk + klogk) -> O((n+k)logk) -> O(2nlogk) -> O(nlogk)?
        // Space O(k)
        // int[][] ans = new int[k][2];
        ...
        int[][] ans = new int[k][2];
        // array format: [x, y, distance]
        // Queue<int[]> maxHeap = new PriorityQueue<>((a, b) -> Integer.compare(b[2], a[2]));
        Queue<int[]> maxHeap = new PriorityQueue<>((a, b) -> Integer.compare(b[0] * b[0] + b[1] * b[1], a[0] * a[0] + a[1] * a[1]));

        for (int[] point : points) {
            int x = point[0], y = point[1];
            // No need to use Math.sqrt here, since if x < y then x^1/2 < y^1/2 when x,y, > 0
            // maxHeap.offer(new int[]{x, y, x*x + y * y});
            maxHeap.offer(point);
            if (maxHeap.size() > k) {
                maxHeap.poll();
            }
